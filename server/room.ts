...
export class Room {
  ...
  //       if (value.collisionShape.type == "Bot") {
  //         damageArtefacts = value.takeAction(
  //           this.gameObjects,
  ...
  private insertInRandomPosition(gameObject: GameObject): void {
    ...
    while (isColliding(gameObject.spawnToleranceShape)) {
      gameObject.setPos(
        getRndInteger(0, this.canvasWidth),
        getRndInteger(0, this.canvasHeight)
      );
      this.spawnCollisionSystem.update();
    }
  ...
  public addNewPlayer(socket: any, data: any): void {
    if (this.gameObjects.get(socket.id)) {
      console.log(
        this.name + " : " + `Player with id ${socket.id} already exists`
      );
      return;
    }
  ...
  public removePlayer(player: any) {
    ...
    if (this.gameObjects.has(player.id)) {
      ...
      this.io
        .in(this.name)
        .emit("remove_player", { id: player.id, x: player.x, y: player.y });
  ...
  public disconnectPlayer(playerID: string): void {
    if (this.gameObjects.has(playerID)) {
      ...
      console.log(
        this.name + " : " + "Disconnected player with ID = " + playerID
      );
    } else {
      console.log(
        this.name +
          " : " +
          "ERROR: Could not find player with ID = " +
          playerID +
          " in room to disconnect"
      );
    }
  }
  ...
  //   for (
  //     let i: number = this.debrisFieldsCount;
  //     i < this.debrisFieldsMax;
  //     i++
  //   ) {
  //     this.addDerbisField();
  //   }
  ...
  public updatePlayerInput(socket: any, data: any): void {
    ...
    if (!this.gameObjects.has(socket.id)) return;
